...
//função pra pegar as imagens dessa API, Infelizmente não consegui tirar o link recebido dos " [ ] " então a API só pega apenas 1 imagem de gatinho
var resposta;
const String URLCatApi = "https://api.thecatapi.com/v1/images/search";
fetchImage() async {
  var response = await http
      .get(Uri.parse(URLCatApi), headers: {"Accept": "application/json"});
  if (response.statusCode == 200) {
    List jsonResponse = json.decode(response.body);
...
class _getRaState extends State<getRa> {
  ...
  Widget build(BuildContext context) {
    fetchImage(); // EXECUTANDO A FUNCAO DAS IMAGENS PEGAS PELA API
    return Scaffold(
